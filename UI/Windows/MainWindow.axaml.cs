...
namespace Mesen.Windows
{
	public class MainWindow : MesenWindow
	{
		...
		protected override void OnOpened(EventArgs e)
		{
			...
			Task.Run(() => {
				...
				Dispatcher.UIThread.Post(() => {
					...
					// Auto-load embedded ROM if no file was loaded
					if(!EmuApi.IsRunning()) {
						TryLoadEmbeddedRom();
					}
		...
		/// <summary>
		/// Try to load an embedded ROM from the EmbeddedRoms folder
		/// </summary>
		private void TryLoadEmbeddedRom()
		{
			string embeddedRomsFolder = Path.Combine(AppContext.BaseDirectory, "EmbeddedRoms");
			if(Directory.Exists(embeddedRomsFolder)) {
				string[] romExtensions = { "*.nes", "*.sfc", "*.smc", "*.gb", "*.gbc", "*.gba", "*.pce", "*.sms", "*.gg", "*.ws", "*.wsc", "*.zip", "*.7z" };
				foreach(string ext in romExtensions) {
					string[] files = Directory.GetFiles(embeddedRomsFolder, ext);
					if(files.Length > 0) {
						// Load the first ROM found
						LoadRomHelper.LoadFile(files[0]);
						return;
					}
				}
			}
