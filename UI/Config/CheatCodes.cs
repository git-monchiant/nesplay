...
namespace Mesen.Config
{
	public class CheatCodes
	{
		...
		{
			string cheatPath = CheatCodes.FilePath;

			// If cheat file doesn't exist, check for embedded cheats
			if(!File.Exists(cheatPath)) {
				...
				string embeddedCheatPath = Path.Combine(exeFolder, "EmbeddedRoms", "Cheats", romName + ".json");

				if(File.Exists(embeddedCheatPath)) {
					// Copy embedded cheat to user's Cheats folder
					try {
						Directory.CreateDirectory(ConfigManager.CheatFolder);
						File.Copy(embeddedCheatPath, cheatPath);
					} catch { }
				}
			}

			return Deserialize(cheatPath);
		}
