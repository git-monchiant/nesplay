...
extern "C" {
	...
	DllExport bool __stdcall LoadRomFromMemory(uint8_t* romData, uint32_t romSize, char* romName)
	{
		_emu->GetGameClient()->Disconnect();
		VirtualFile romFile(romData, romSize, romName ? string(romName) : "embedded.rom");
		return _emu->LoadRom(romFile, VirtualFile());
	}
