...
#define nesplay_psp_round_page(x) ((((uintptr_t)(x)) + MEM_PAGE_MASK) & ~(MEM_PAGE_MASK))
...
#if defined(_WIN32) && NESPLAY_PSP_ARCH(AMD64)
...
// provides exactly the primitive operations that NESPLAY_PSP needs.
...
#if NESPLAY_PSP_ARCH(AMD64)
	if ((uintptr_t)&hint_location > 0xFFFFFFFFULL) {
		size_t aligned_size = nesplay_psp_round_page(size);
...
void *AllocateExecutableMemory(size_t size) {
	...
	if (!map_hint) {
		if ((uintptr_t) &hint_location > 0xFFFFFFFFULL)
			map_hint = (char*)nesplay_psp_round_page(&hint_location) - 0x20000000; // 0.5gb lower than our approximate location
		else
			map_hint = (char*)0x20000000; // 0.5GB mark in memory
	} else if ((uintptr_t) map_hint > 0xFFFFFFFFULL) {
		map_hint -= nesplay_psp_round_page(size); /* round down to the next page if we're in high memory */
	}
#endif
	...
	if (ptr == MAP_FAILED) {
		...
		map_hint += nesplay_psp_round_page(size);
...
void *AllocateMemoryPages(size_t size, uint32_t memProtFlags) {
	...
	size = nesplay_psp_round_page(size);
#if NESPLAY_PSP_PLATFORM(UWP)
...
#if NESPLAY_PSP_PLATFORM(IOS) || NESPLAY_PSP_PLATFORM(UWP) || defined(__OpenBSD__)
	return true;
#elif NESPLAY_PSP_PLATFORM(MAC) && NESPLAY_PSP_ARCH(ARM64)
	return true;
#else
...
#endif // !NESPLAY_PSP_PLATFORM(SWITCH)
