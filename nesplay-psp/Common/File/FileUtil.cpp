...
#if NESPLAY_PSP_PLATFORM(UWP) && !defined(__LIBRETRO__)
...
#elif (defined(__APPLE__) && !NESPLAY_PSP_PLATFORM(IOS)) || defined(__linux__) || defined(KERN_PROC_PATHNAME)
		char program_path[4096]{};
		uint32_t program_path_size = sizeof(program_path) - 1;

#if defined(__linux__)
		if (readlink("/proc/self/exe", program_path, program_path_size) > 0)
#elif defined(__APPLE__) && !NESPLAY_PSP_PLATFORM(IOS)
