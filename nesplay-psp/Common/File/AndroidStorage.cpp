...
#if NESPLAY_PSP_PLATFORM(ANDROID) && !defined(__LIBRETRO__)
...
void Android_RegisterStorageCallbacks(JNIEnv * env, jobject obj) {
	jclass localClass = env->FindClass("org/nesplay_psp/nesplay_psp/ContentUri");
...
int Android_OpenContentUriFd(std::string_view filename, Android_OpenContentUriMode mode) {
	...
	*/
	...
	// NESPLAY_PSP adds an ending slash to directories before looking them up.
