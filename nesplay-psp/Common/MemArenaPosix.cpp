...
#if !defined(_WIN32) && !defined(ANDROID) && !defined(__APPLE__) && !NESPLAY_PSP_PLATFORM(SWITCH)
...
bool MemArena::GrabMemSpace(size_t size) {
	...
	bool is_shm = false;
	for (int i = 0; i < 256; ++i) {
		snprintf(ram_temp_filename, sizeof(ram_temp_filename), "/nesplay_psp_%d.ram", i);
...
#if NESPLAY_PSP_ARCH(64BIT) && !defined(USE_ASAN) && !defined(NO_MMAP)
