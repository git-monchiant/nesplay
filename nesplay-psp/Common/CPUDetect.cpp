...
#if (NESPLAY_PSP_ARCH(X86) || NESPLAY_PSP_ARCH(AMD64)) && !defined(__EMSCRIPTEN__)
...
void CPUInfo::Detect() {
...
#if NESPLAY_PSP_ARCH(X86)
	Mode64bit = false;
#elif NESPLAY_PSP_ARCH(AMD64)
	Mode64bit = true;
	OS64bit = true;
#endif
	num_cores = 1;
...
#elif defined(_WIN32) && NESPLAY_PSP_ARCH(X86)
	BOOL f64 = false;
	...
	*((int *)(cpu_string + 8)) = cpu_id[2];
	...
	if (max_std_fn >= 1) {
		...
		// Detect people unfortunate enough to be running NESPLAY_PSP on an Atom
...
#elif NESPLAY_PSP_PLATFORM(LINUX)
...
#endif // NESPLAY_PSP_ARCH(X86) || NESPLAY_PSP_ARCH(AMD64)

const char *GetCompilerABI() {
#if NESPLAY_PSP_ARCH(ARMV7)
	return "armeabi-v7a";
#elif NESPLAY_PSP_ARCH(ARM64)
	return "arm64";
#elif NESPLAY_PSP_ARCH(X86)
	return "x86";
#elif NESPLAY_PSP_ARCH(AMD64)
	return "x86-64";
