...
Choice *TabHolder::AddBack(UIScreen *parent) {
	if (tabContainer_) {
		auto di = GetI18NCategory(I18NCat::DIALOG);
		tabContainer_->Add(new UI::Spacer(8.0f));
		Choice *back = tabContainer_->Add(new Choice(di->T("Back"), ImageID("I_NAVIGATE_BACK"), new LinearLayoutParams(FILL_PARENT, WRAP_CONTENT, 0.0f, Margins(0, 0, 10, 10))));
		back->OnClick.Handle<UIScreen>(parent, &UIScreen::OnBack);
		return back;
	}
	return nullptr;
}
...
void TabHolder::SetAutoSelectOnFocus(bool autoSelect) {
	tabStrip_->SetAutoSelectOnFocus(autoSelect);
}
...
void ChoiceStrip::AddChoice(std::string_view title, ImageID imageId) {
	...
	c->SetFocusCallback([this](int focusFlags, View *v) { OnChoiceFocus(focusFlags, v); });
	Add(c);
...
void ChoiceStrip::OnChoiceFocus(int focusFlags, View *v) {
	if (!autoSelectOnFocus_ || !(focusFlags & FF_GOTFOCUS))
		return;
	for (int i = 0; i < (int)choices_.size(); i++) {
		if (choices_[i] == v && i != selected_) {
			SetSelection(i, true);
			break;
		}
	}
