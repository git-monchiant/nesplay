...
#if !NESPLAY_PSP_ARCH(X86)  // Doesn't work on AMD for some reason. See issue #17787
...
void GLQueueRunner::RunInitSteps(const FastVec<GLRInitStep> &steps, bool skipGLCalls) {
	...
	if (false && allocatedTextures) {
		...
		// This detects that, rather than looking like NESPLAY_PSP is broken.
...
#if NESPLAY_PSP_PLATFORM(IOS) && !defined(__LIBRETRO__)
	bindDefaultFBO();
#endif
