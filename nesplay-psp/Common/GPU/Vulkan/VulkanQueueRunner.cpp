...
// Debug help: adb logcat -s DEBUG AndroidRuntime NESPLAY_PSPNativeActivity NESPLAY_PSP NativeGLView NativeRenderer NativeSurfaceView PowerSaveModeReceiver InputDeviceState Nesplay_PspActivity CameraHelper
			...
			for (int j = i + 1; j < (int)steps.size(); j++) {
				...
				case VKRStepType::RENDER:
					if (steps[j]->render.framebuffer == fb) {
						// Prevent Unknown's example case from https://github.com/hrydgard/nesplay_psp/pull/12242
