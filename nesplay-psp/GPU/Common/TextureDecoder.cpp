...
static u32 QuickTexHashNEON(const void *checkp, u32 size) {
	...
	if (((intptr_t)checkp & 0xf) == 0 && (size & 0x3f) == 0) {
#if NESPLAY_PSP_PLATFORM(IOS) || NESPLAY_PSP_ARCH(ARM64) || defined(_MSC_VER) || !NESPLAY_PSP_ARCH(ARMV7)
...
#endif  // NESPLAY_PSP_ARCH(ARM_NEON)

#if NESPLAY_PSP_ARCH(LOONGARCH64_LSX)
...
#endif // NESPLAY_PSP_ARCH(LOONGARCH64_LSX)
...
#elif NESPLAY_PSP_ARCH(LOONGARCH64_LSX)
