...
#if NESPLAY_PSP_PLATFORM(WINDOWS) && (defined(_MSC_VER) || defined(__clang__) || defined(__INTEL_COMPILER))
...
// spills, but that, hopefully, does not affect NESPLAY_PSP (modern GCC+Linux
// is 16-byte aligned on x86, and MinGW is not a supported NESPLAY_PSP target).
...
public:
	union
	{
		...
		__m128i ivec;
		__m128 vec;
#elif NESPLAY_PSP_ARCH(ARM_NEON)
		int32x4_t ivec;
		float32x4_t vec;
#endif
	};
