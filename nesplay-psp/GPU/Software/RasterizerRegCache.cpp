// Copyright (c) 2021- NESPLAY_PSP Project.
...
#elif NESPLAY_PSP_ARCH(ARM64_NEON)
...
#if NESPLAY_PSP_ARCH(ARM64_NEON)
	: fp(this)
#endif
{
	AllocCodeSpace(size);
...
#if defined(_WIN32) && (NESPLAY_PSP_ARCH(X86) || NESPLAY_PSP_ARCH(AMD64)) && !NESPLAY_PSP_PLATFORM(UWP)
	using namespace Gen;
	for (int i = 0; i < 100; i++) {
		MOV(32, R(EAX), R(EBX));
		RET();
	}
#elif NESPLAY_PSP_ARCH(ARM)
	BKPT(0);
	BKPT(0);
#endif
}
