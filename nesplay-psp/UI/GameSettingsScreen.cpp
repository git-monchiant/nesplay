...
#if NESPLAY_PSP_PLATFORM(MAC) || NESPLAY_PSP_PLATFORM(IOS)
#include "UI/DarwinFileSystemServices.h"
#endif

#if defined(_WIN32) && !NESPLAY_PSP_PLATFORM(UWP)
...
#if NESPLAY_PSP_PLATFORM(ANDROID)
...
GameSettingsScreen::~GameSettingsScreen() {
	Reporting::Enable(enableReports_, "report.nesplay_psp.org");
...
void GameSettingsScreen::HandleL2(bool pressed) {
	if (!pressed) {
		dismissInputReleased_ = true;
	}
	// Time-based debounce: ignore L2 for the first 0.3s after screen creation.
	// This prevents analog trigger bounce from immediately closing the screen.
	if (pressed && dismissInputReleased_ && time_now_d() > createdTime_ + 0.3) {
		TriggerFinish(DR_BACK);
		dismissInputReleased_ = false;
	}
}

bool GameSettingsScreen::key(const KeyInput &key) {
	if (key.keyCode == NKCODE_ESCAPE) {
		HandleL2(!!(key.flags & KeyInputFlags::DOWN));
		return true;
	}
	if (key.keyCode == NKCODE_BUTTON_L2) {
	...
	return UITabbedBaseDialogScreen::key(key);
}

void GameSettingsScreen::axis(const AxisInput &axis) {
	if (axis.axisId == JOYSTICK_AXIS_LTRIGGER) {
		HandleL2(axis.value > 0.5f);
	}
	UITabbedBaseDialogScreen::axis(axis);
}

void GameSettingsScreen::CreateExtraButtons(UI::ViewGroup *verticalLayout, int margins) {
	using namespace UI;
	verticalLayout->Add(new Spacer(8.0f));
	if (!gamePath_.empty()) {
		// In-game: "Back" resumes game (default focus), "Exit to menu" returns to MainScreen
		auto di = GetI18NCategory(I18NCat::DIALOG);
		auto pa = GetI18NCategory(I18NCat::PAUSE);
		Choice *backChoice = verticalLayout->Add(new Choice(di->T("Back"), ImageID("I_NAVIGATE_BACK"),
			new LinearLayoutParams(FILL_PARENT, WRAP_CONTENT, 0.0f, Margins(0, 0, margins, margins))));
		backChoice->OnClick.Handle<UIScreen>(this, &UIScreen::OnBack);
		backButton_ = backChoice;
		Choice *exitChoice = verticalLayout->Add(new Choice(pa->T("Exit to menu"), ImageID("I_EXIT"),
			new LinearLayoutParams(FILL_PARENT, WRAP_CONTENT, 0.0f, Margins(0, 0, margins, margins))));
		exitChoice->OnClick.Add([this](UI::EventParams &e) {
			TriggerFinish(DR_OK);
		});
	} else {
		// Menu mode: "Exit" exits the application
		auto mm = GetI18NCategory(I18NCat::MAINMENU);
		Choice *exitChoice = verticalLayout->Add(new Choice(mm->T("Exit"), ImageID("I_EXIT"),
			new LinearLayoutParams(FILL_PARENT, WRAP_CONTENT, 0.0f, Margins(0, 0, margins, margins))));
		exitChoice->OnClick.Handle(this, &GameSettingsScreen::OnExitApp);
	}
}

void GameSettingsScreen::OnExitApp(UI::EventParams &e) {
	if (!g_Config.Save("GameSettingsScreen::OnExitApp")) {
...
void GameSettingsScreen::CreateViews() {
	...
	// In-game: defer Back button focus to update() so it happens after layout init.
	if (!gamePath_.empty() && backButton_) {
		needsFocusBack_ = true;
	}
}

void GameSettingsScreen::update() {
	UITabbedBaseDialogScreen::update();
	if (needsFocusBack_ && backButton_) {
		UI::EnableFocusMovement(true);
		UI::SetFocusedView(backButton_, true);
		needsFocusBack_ = false;
	}
}
...
void GameSettingsScreen::CreateTabs() {
	...
	// Save State tab - only show when a game is running
	if (!gamePath_.empty()) {
		AddTab("GameSettingsSaveState", ms->T("Save State"), ImageID("I_SDCARD"), [this](UI::LinearLayout *parent) {
			CreateSaveStateSettings(parent);
		});
	}
...
#if NESPLAY_PSP_PLATFORM(ANDROID) || NESPLAY_PSP_PLATFORM(IOS)
...
#if NESPLAY_PSP_PLATFORM(WINDOWS) && !NESPLAY_PSP_PLATFORM(UWP)
...
void GameSettingsScreen::CreateAudioSettings(UI::ViewGroup *audioSettings) {
	...
	audioSettings->Add(new SettingHint(di->T("This change will not take effect until NESPLAY_PSP is restarted.")));
#endif
...
#if defined(USING_WIN_UI) || (NESPLAY_PSP_PLATFORM(LINUX) && !NESPLAY_PSP_PLATFORM(ANDROID))
...
#if defined(USING_WIN_UI) || defined(SDL) || NESPLAY_PSP_PLATFORM(ANDROID)
...
void GameSettingsScreen::CreateNetworkingSettings(UI::ViewGroup *networkingSettings) {
	...
	Choice *wiki = networkingSettings->Add(new Choice(n->T("Open NESPLAY_PSP Multiplayer Wiki Page"), ImageID("I_LINK_OUT")));
...
#if (!defined(MOBILE_DEVICE) && !defined(USING_QT_UI)) || defined(USING_QT_UI) || NESPLAY_PSP_PLATFORM(ANDROID) // Missing only iOS?
...
#if (!defined(MOBILE_DEVICE) && !defined(USING_QT_UI)) || defined(USING_QT_UI) || NESPLAY_PSP_PLATFORM(ANDROID)
...
#if defined(USING_QT_UI) || NESPLAY_PSP_PLATFORM(ANDROID)
...
void GameSettingsScreen::CreateSystemSettings(UI::ViewGroup *systemSettings) {
	...
	const Path &NESPLAY_PSPpath = File::GetExeDirectory();
	const Path installedFile = NESPLAY_PSPpath / "installed.txt";
	installed_ = File::Exists(installedFile);
	otherinstalled_ = false;
	if (!installed_ && myDocsExists) {
		if (File::CreateEmptyFile(NESPLAY_PSPpath / "installedTEMP.txt")) {
			// Disable the setting whether cannot create & delete file
			if (!(File::Delete(NESPLAY_PSPpath / "installedTEMP.txt")))
	...
	systemSettings->Add(new ItemHeader(sy->T("Help the NESPLAY_PSP team")));
	...
	systemSettings->Add(new CheckBox(&g_Config.bCheckForNewVersion, sy->T("VersionCheck", "Check for new versions of NESPLAY_PSP")));
...
void GameSettingsScreen::CreateSaveStateSettings(UI::ViewGroup *saveStateSettings) {
	using namespace UI;

	auto pa = GetI18NCategory(I18NCat::PAUSE);

	std::string saveStatePrefix = SaveState::GetGamePrefix(g_paramSFO);
	SaveState::Rescan(saveStatePrefix);

	saveStateSettings->Add(new ItemHeader(pa->T("Save State")));

	for (int i = 0; i < g_Config.iSaveStateSlotCount; i++) {
		bool hasSave = SaveState::HasSaveInSlot(saveStatePrefix, i);
		std::string prefix = saveStatePrefix;
		int slot = i;

		// Single row: [Slot N - date]  [Save]  [Load]
		LinearLayout *row = new LinearLayout(ORIENT_HORIZONTAL, new LinearLayoutParams(FILL_PARENT, WRAP_CONTENT, Margins(12, 4, 12, 4)));
		row->SetSpacing(10.0f);
		saveStateSettings->Add(row);

		// Slot label
		std::string slotLabel = StringFromFormat("Slot %d", i + 1);
		if (hasSave) {
			std::string dateStr = SaveState::GetSlotDateAsString(saveStatePrefix, i);
			if (!dateStr.empty()) {
				slotLabel += "  " + dateStr;
			}
		} else {
		...
		TextView *label = row->Add(new TextView(slotLabel, new LinearLayoutParams(1.0f, Gravity::G_VCENTER)));
		label->SetShadow(true);

		// Save button
		Choice *saveBtn = row->Add(new Choice(pa->T("Save State"), new LinearLayoutParams(WRAP_CONTENT, WRAP_CONTENT, 0.0f, Gravity::G_VCENTER)));
		saveBtn->OnClick.Add([prefix, slot](UI::EventParams &e) {
			g_Config.iCurrentStateSlot = slot;
			SaveState::SaveSlot(prefix, slot, &AfterSaveStateAction);
		});

		// Load button (only if save exists)
		if (hasSave) {
			Choice *loadBtn = row->Add(new Choice(pa->T("Load State"), new LinearLayoutParams(WRAP_CONTENT, WRAP_CONTENT, 0.0f, Gravity::G_VCENTER)));
			loadBtn->OnClick.Add([prefix, slot](UI::EventParams &e) {
				g_Config.iCurrentStateSlot = slot;
				SaveState::LoadSlot(prefix, slot, &AfterSaveStateAction);
			});
		}
	}
}
...
void GameSettingsScreen::OnAdhocGuides(UI::EventParams &e) {
	auto n = GetI18NCategory(I18NCat::NETWORKING);
	std::string url(n->T("MultiplayerHowToURL", "https://github.com/hrydgard/nesplay_psp/wiki/How-to-play-multiplayer-games-with-NESPLAY_PSP"));
...
void GameSettingsScreen::OnMemoryStickMyDoc(UI::EventParams &e) {
	...
	installed_ = File::Exists(installedFile);
	if (otherinstalled_) {
		File::Delete(NESPLAY_PSPpath / "installed.txt");
		File::CreateEmptyFile(NESPLAY_PSPpath / "installed.txt");
		otherinstalled_ = false;
		const std::string myDocsPath = W32Util::UserDocumentsPath() + "/NESPLAY_PSP/";
		g_Config.memStickDirectory = Path(myDocsPath);
	} else if (installed_) {
		File::Delete(NESPLAY_PSPpath / "installed.txt");
		installed_ = false;
		g_Config.memStickDirectory = NESPLAY_PSPpath / "memstick";
	} else {
		FILE *f = File::OpenCFile(NESPLAY_PSPpath / "installed.txt", "wb");
		if (f) {
			fclose(f);
		}
...
void GameSettingsScreen::OnMemoryStickOther(UI::EventParams &e) {
	...
	if (otherinstalled_) {
		...
		std::string folder = W32Util::BrowseForFolder2(MainWindow::GetHWND(), di->T("Choose NESPLAY_PSP save folder"), initialPath);
		if (folder.size()) {
...
void GameSettingsScreen::TriggerRestartOrDo(std::function<void()> callback) {
	auto di = GetI18NCategory(I18NCat::DIALOG);
	screenManager()->push(new UI::MessagePopupScreen(di->T("Restart"), di->T("Changing this setting requires NESPLAY_PSP to restart."), di->T("Restart"), di->T("Cancel"), [=](bool yes) {
		if (yes) {
...
void GameSettingsScreen::OnRestoreDefaultSettings(UI::EventParams &e) {
	...
	if (g_Config.IsGameSpecific()) {
		...
		auto sy = GetI18NCategory(I18NCat::SYSTEM);
		screenManager()->push(
			new UI::MessagePopupScreen(sy->T("Restore Default Settings"), dev->T("RestoreGameDefaultSettings", "Are you sure you want to restore the game-specific settings back to the nesplay_psp defaults?\n"),
...
void RestoreSettingsScreen::CreatePopupContents(UI::ViewGroup *parent) {
	...
	std::string_view text = dev->T(
		...
		"Restore these settings back to their defaults?\nYou can't undo this.\nPlease restart NESPLAY_PSP after restoring settings.");
