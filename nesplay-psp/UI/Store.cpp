...
const char *storeBaseUrlHttp = "http://store.nesplay_psp.org/";
const char *storeBaseUrlHttps = "https://store.nesplay_psp.org/";
...
void ProductView::CreateViews() {
	...
	if (!entry_.license.empty()) {
		...
		horiz->Add(new Button(di->T("More info"), new LinearLayoutParams(0.0, Gravity::G_VCENTER)))->OnClick.Add([this](UI::EventParams) {
			std::string url = StringFromFormat("https://www.nesplay_psp.org/docs/reference/homebrew-store-distribution/#%s", entry_.file.c_str());
...
#endif
			e.hidden = false;  // NOTE: Handling of the "hidden" flag is broken in old versions of NESPLAY_PSP. Do not use.
