...
static void AddExplanation(UI::ViewGroup *viewGroup, MemStickScreen::Choice choice, UI::View *extraView = nullptr) {
	...
	case MemStickScreen::CHOICE_STORAGE_ROOT:
		// Old school choice
		holder->Add(new TextView(iz->T("DataWillStay", "Data will stay even if you uninstall NESPLAY_PSP"), flags, false))->SetBullet(true);
		holder->Add(new TextView(iz->T("DataCanBeShared", "Data can be shared between NESPLAY_PSP regular/Gold"), flags, false))->SetBullet(true);
	...
	case MemStickScreen::CHOICE_PRIVATE_DIRECTORY:
		...
		holder->Add(new TextView(iz->T("DataWillBeLostOnUninstall", "Warning! Data will be lost when you uninstall NESPLAY_PSP!"), flags, false))->SetBullet(true);
		holder->Add(new TextView(iz->T("DataCannotBeShared", "Data CANNOT be shared between NESPLAY_PSP regular/Gold!"), flags, false))->SetBullet(true);
#if !NESPLAY_PSP_PLATFORM(UWP)
		if (System_GetPropertyBool(SYSPROP_APP_GOLD)) {
			holder->Add(new TextView(iz->T("USBAccessThroughGold", "USB access through Android/data/org.nesplay_psp.nesplay_pspgold/files"), flags, false))->SetBullet(true);
		} else {
			holder->Add(new TextView(iz->T("USBAccessThrough", "USB access through Android/data/org.nesplay_psp.nesplay_psp/files"), flags, false))->SetBullet(true);
		}
#endif
		break;
...
void MemStickScreen::CreateViews() {
	...
	if (initialSetup_) {
		...
		mainColumn->Add(new TextView(ms->T("Welcome to NESPLAY_PSP!"), ALIGN_LEFT, false));
	}
...
void MemStickScreen::OnHelp(UI::EventParams &params) {
	...
	System_LaunchUrl(LaunchUrlType::BROWSER_URL, "https://www.nesplay_psp.org/guide_storage.html");
}
...
#if NESPLAY_PSP_PLATFORM(ANDROID) || NESPLAY_PSP_PLATFORM(SWITCH)
...
void ConfirmMemstickMoveScreen::CreateViews() {
	...
	if (!initialSetup_) {
		leftColumn->Add(new NoticeView(NoticeLevel::INFO, ms->T("NESPLAY_PSP will restart after the change"), ""));
	}
