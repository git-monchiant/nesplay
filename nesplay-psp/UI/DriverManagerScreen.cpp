...
void DriverManagerScreen::CreateDriverTab(UI::ViewGroup *drivers) {
	...
	drivers->Add(new Choice(di->T("More info")))->OnClick.Add([=](UI::EventParams &e) {
		System_LaunchUrl(LaunchUrlType::BROWSER_URL, "https://www.nesplay_psp.org/docs/reference/custom-drivers/");
	});
...
void DriverManagerScreen::OnCustomDriverChange(UI::EventParams &e) {
	...
	screenManager()->push(new UI::MessagePopupScreen(gr->T("Drivers"), di->T("Changing this setting requires NESPLAY_PSP to restart."), di->T("Restart"), di->T("Cancel"), [=](bool yes) {
		if (yes) {
