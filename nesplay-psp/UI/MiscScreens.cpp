...
void PromptScreen::CreateViews() {
	...
	if (!noButton) {
		...
		// Sneak in the version of NESPLAY_PSP in the bottom left corner, for debug-reporting user screenshots.
...
#if !(defined(USING_QT_UI) || NESPLAY_PSP_PLATFORM(WINDOWS) || NESPLAY_PSP_PLATFORM(ANDROID))
...
const float logoScreenSeconds = 3.0f;
...
void LogoScreen::DrawForeground(UIContext &dc) {
	...
	// Fade in alpha
	float alpha = std::min((float)sinceStart_ * 2.0f, 1.0f);
	uint32_t iconColor = colorAlpha(0xFFFFFFFF, alpha);

	// Scale to 30% of screen height, same as nesplay
	float imgW = 0, imgH = 0;
	dc.Draw()->MeasureImage(ImageID("I_ICON"), &imgW, &imgH);
	float iconScale = (imgH > 0) ? (bounds.h * 0.3f / imgH) : 1.0f;
	dc.Draw()->DrawImage(ImageID("I_ICON"), bounds.centerX(), bounds.centerY(), iconScale, iconColor, ALIGN_CENTER);
...
void CreditsScreen::CreateDialogViews(UI::ViewGroup *parent) {
	...
	int rightYOffset = 0;
	// External links removed for nesplay-psp kiosk build
}
...
void CreditsScroller::Draw(UIContext &dc) {
	...
	std::string_view credits[] = {
		...
		"",
		"",
		cr->T("info1", "nesplay-psp is only intended to play games you own."),
	...
	char temp[256];
	snprintf(temp, sizeof(temp), "nesplay-psp %s", NESPLAY_PSP_GIT_VERSION);
