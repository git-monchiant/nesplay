	...
	if (WebServerRunning(WebServerFlags::DEBUGGER)) {
		items->Add(new Choice(dev->T("Remote debugger")))->OnClick.Add([](UI::EventParams &e) {
			...
			if (g_Config.bRemoteDebuggerLocal) {
				...
				System_LaunchUrl(LaunchUrlType::BROWSER_URL, "http://nesplay_psp-debugger.unknownbrackets.org/cpu");  // NOTE: https doesn't work
			}
		});
	}
...
const std::string framedumpsBaseUrl = "http://framedump.nesplay_psp.org/repro/";
...
void RecreateActivity() {
	...
	if (System_GetPropertyInt(SYSPROP_SYSTEMVERSION) >= SYSTEM_JELLYBEAN) {
		...
		auto gr = GetI18NCategory(I18NCat::GRAPHICS);
		System_Toast(gr->T_cstr("Must Restart", "You must restart NESPLAY_PSP for this change to take effect"));
	}
}
