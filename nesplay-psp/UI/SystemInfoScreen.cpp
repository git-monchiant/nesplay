...
static void CopySummaryToClipboard(Draw::DrawContext *draw) {
	...
	build = "Debug";
#endif
	w.W(NESPLAY_PSP_GIT_VERSION).C(" ").W(build).endl();
...
void SystemInfoScreen::CreateDeviceInfoTab(UI::LinearLayout *deviceSpecs) {
	...
	build = si->T("Debug");
#endif
	osInformation->Add(new InfoItem(si->T("nesplay-psp build"), build));
...
#if NESPLAY_PSP_PLATFORM(ANDROID) || NESPLAY_PSP_PLATFORM(UWP)
