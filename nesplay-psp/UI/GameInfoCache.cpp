...
// Forward declarations for cover art support
static bool ReadLocalFileToString(const Path &path, std::string *contents, std::mutex *mtx);
static std::string GetCoverRegion(const std::string &discId);
...
bool GameInfo::Delete() {
	...
	case IdentifiedFileType::NESPLAY_PSP_GE_DUMP:
		{
...
u64 GameInfo::GetGameSavedataSizeInBytes() {
	if (fileType == IdentifiedFileType::PSP_SAVEDATA_DIRECTORY || fileType == IdentifiedFileType::NESPLAY_PSP_SAVESTATE) {
		return 0;
	}
...
void GameInfo::FinishPendingTextureLoads(Draw::DrawContext *draw) {
	...
	// Cover art download from HexFlow-Covers (GitHub)
	if (coverDownloadPending.load(std::memory_order_relaxed) && !coverDownloadStarted_ && !id.empty()) {
		coverDownloadStarted_ = true;
		std::string url = "https://raw.githubusercontent.com/Andiweli/HexFlow-Covers/main/Covers/PSP/" + id + ".png";
		Path coverPath = GetSysDirectory(DIRECTORY_COVERS) / (id + ".png");
		INFO_LOG(Log::Loader, "Downloading cover art for %s from %s", id.c_str(), url.c_str());
		coverDownloadRequest_ = g_DownloadManager.StartDownloadWithCallback(
			url, coverPath, http::RequestFlags::Default,
			[this, draw](http::Request &req) {
				coverDownloadPending.store(false, std::memory_order_relaxed);
				if (req.ResultCode() == 200) {
					// Read the downloaded cover and refresh the icon texture
					Path coverPath = GetSysDirectory(DIRECTORY_COVERS) / (id + ".png");
					std::string newData;
					if (ReadLocalFileToString(coverPath, &newData, nullptr)) {
						std::lock_guard<std::mutex> guard(lock);
						icon.data = std::move(newData);
						if (icon.texture) {
							icon.texture->Release();
							icon.texture = nullptr;
						}
						icon.timeLoaded = 0.0;
						icon.dataLoaded = true;
					}
				} else {
					INFO_LOG(Log::Loader, "Cover art download failed for %s (HTTP %d)", id.c_str(), req.ResultCode());
				}
				coverDownloadRequest_.reset();
			});
	}
}
...
static bool LoadCoverImage(GameInfo *info, GameInfoTex *tex) {
	if (info->id.empty()) {
		return false;
	}
	Path coverPath = GetSysDirectory(DIRECTORY_COVERS) / (info->id + ".png");
	if (ReadLocalFileToString(coverPath, &tex->data, &info->lock)) {
		tex->dataLoaded = true;
		return true;
	}
	// Also check .jpg
	coverPath = GetSysDirectory(DIRECTORY_COVERS) / (info->id + ".jpg");
	if (ReadLocalFileToString(coverPath, &tex->data, &info->lock)) {
		tex->dataLoaded = true;
		return true;
	}
...
static std::string GetCoverRegion(const std::string &discId) {
	if (discId.size() < 4) return "US";
	std::string prefix = discId.substr(0, 4);
	if (prefix == "UCUS" || prefix == "ULUS" || prefix == "NPUG" || prefix == "NPUH") return "US";
	if (prefix == "UCES" || prefix == "ULES" || prefix == "NPEG" || prefix == "NPEH") return "EN";
	if (prefix == "UCJS" || prefix == "ULJS" || prefix == "NPJG" || prefix == "NPJH") return "JA";
	if (prefix == "UCAS" || prefix == "ULAS") return "KO";
	return "US";
}
		...
		case IdentifiedFileType::PSP_PBP_DIRECTORY:
			{
				...
				// Then, ICON0.PNG. Check covers folder first.
				if (flags_ & GameInfoFlags::ICON) {
					if (LoadCoverImage(info_.get(), &info_->icon)) {
						// Cover art found in COVERS folder
					} else if (LoadReplacementImage(info_.get(), &info_->icon, "icon.png")) {
						// Replacement texture found
						info_->coverDownloadPending = true;
		...
		case IdentifiedFileType::PSP_UMD_VIDEO_ISO:
			{
				...
				// Check covers folder first, then replacement textures, then ICON0.PNG from ISO.
				if (flags_ & GameInfoFlags::ICON) {
					if (LoadCoverImage(info_.get(), &info_->icon)) {
						// Cover art found in COVERS folder
					} else if (LoadReplacementImage(info_.get(), &info_->icon, "icon.png")) {
