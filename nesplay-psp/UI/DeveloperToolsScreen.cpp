...
#if !NESPLAY_PSP_ARCH(X86) && !NESPLAY_PSP_ARCH(AMD64) && !NESPLAY_PSP_ARCH(ARM64)
	core->HideChoice(3);
#endif
...
#ifdef _DEBUG
	// Untranslated string because this is debug mode only, only for NESPLAY_PSP developers.
...
#if !NESPLAY_PSP_PLATFORM(ANDROID) && !NESPLAY_PSP_PLATFORM(IOS) && !NESPLAY_PSP_PLATFORM(SWITCH)
