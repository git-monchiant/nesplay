...
#if NESPLAY_PSP_ARCH(ARM) && defined(__ANDROID__)
#include "../../android/jni/ArmEmitterTest.h"
#elif NESPLAY_PSP_ARCH(ARM64) && defined(__ANDROID__)
...
void NativeGetAppInfo(std::string *app_dir_name, std::string *app_nice_name, bool *landscape, std::string *version) {
	*app_nice_name = "nesplay-psp";
	*app_dir_name = "nesplay-psp";
	*landscape = true;
	*version = NESPLAY_PSP_GIT_VERSION;
...
#if defined(USING_WIN_UI) && !NESPLAY_PSP_PLATFORM(UWP)
...
void NativeInit(int argc, const char *argv[], const char *savegame_dir, const char *external_dir, const char *cache_dir) {
...
#endif
#if !defined(MOBILE_DEVICE) && !defined(_WIN32) && !NESPLAY_PSP_PLATFORM(SWITCH)
	g_VFS.Register("", new DirectoryReader(File::GetExeDirectory() / "assets"));
	g_VFS.Register("", new DirectoryReader(File::GetExeDirectory()));
	g_VFS.Register("", new DirectoryReader(Path("/usr/local/share/nesplay_psp/assets")));
	g_VFS.Register("", new DirectoryReader(Path("/usr/local/share/games/nesplay_psp/assets")));
	g_VFS.Register("", new DirectoryReader(Path("/usr/share/nesplay_psp/assets")));
	g_VFS.Register("", new DirectoryReader(Path("/usr/share/games/nesplay_psp/assets")));
#endif

#if NESPLAY_PSP_PLATFORM(SWITCH)
...
#if NESPLAY_PSP_PLATFORM(WINDOWS) || NESPLAY_PSP_PLATFORM(MAC)
...
#elif NESPLAY_PSP_PLATFORM(UWP) && !defined(__LIBRETRO__)
	Path memstickDirFile = g_Config.internalDataDirectory / "memstick_dir.txt";
	if (File::Exists(memstickDirFile)) {
		INFO_LOG(Log::System, "Reading '%s' to find memstick dir.", memstickDirFile.c_str());
		std::string memstickDir;
	...
	g_Config.memStickDirectory = g_Config.internalDataDirectory / "config/nesplay_psp";
	...
	g_Config.memStickDirectory = Path(config) / "nesplay_psp";
...
#if !NESPLAY_PSP_PLATFORM(WINDOWS) && !defined(ANDROID)
		PSP_CoreParameter().pixelWidth = g_display.pixel_xres;
		PSP_CoreParameter().pixelHeight = g_display.pixel_yres;
...
// Right now we just store secrets in separate files next to nesplay_psp.ini. The important thing is keeping them out of it
// since we often ask people to post or send the ini for debugging.
static Path GetSecretPath(std::string_view nameOfSecret) {
	return GetSysDirectory(DIRECTORY_SYSTEM) / ("nesplay_psp_" + std::string(nameOfSecret) + ".dat");
}
