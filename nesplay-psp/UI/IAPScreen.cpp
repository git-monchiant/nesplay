// NOTE: This currently only used on iOS, to present the availablility of getting NESPLAY_PSP Gold through IAP.
...
void IAPScreen::CreateViews() {
	...
	if (portrait) {
		root_->Add(new TopBar(*screenManager()->getUIContext(), TopBarFlags::Default, di->T("NESPLAY_PSP Gold")));
	...
	appTitle->Add(new TextView("NESPLAY_PSP Gold", new LinearLayoutParams(1.0f, Gravity::G_VCENTER)));

	leftColumnItems->Add(appTitle);
	leftColumnItems->Add(new Spacer(30.0f));
	if (!bought) {
		leftColumnItems->Add(new Spacer(20.0f));
		leftColumnItems->Add(new TextView(di->T("GoldOverview1", "Buy NESPLAY_PSP Gold to support development!")))->SetAlign(FLAG_WRAP_TEXT);
		...
		leftColumnItems->Add(new TextView(di->T("GoldThankYou", "Thank you for supporting the NESPLAY_PSP project!")))->SetAlign(FLAG_WRAP_TEXT);
	}
	...
	if (!bought) {
		ImageID image;
#if 1 || NESPLAY_PSP_PLATFORM(ANDROID)
		image = ImageID("I_LOGO_PLAY_STORE");
#elif NESPLAY_PSP_PLATFORM(IOS)
		image = ImageID("I_LOGO_APP_STORE");
#endif
		Choice *buyButton = rightColumnItems->Add(new Choice(mm->T("Buy NESPLAY_PSP Gold"), image));
		...
		buyButton->OnClick.Add([this, requesterToken](UI::EventParams &) {
			...
			if (useIAP_) {
				System_IAPMakePurchase(requesterToken, "org.nesplay_psp.gold", [this](const char *responseString, int intValue) {
					INFO_LOG(Log::System, "NESPLAY_PSP Gold purchase successful!");
					auto di = GetI18NCategory(I18NCat::DIALOG);
					g_OSD.Show(OSDType::MESSAGE_SUCCESS, di->T("GoldThankYou", "Thank you for supporting the NESPLAY_PSP project!"), 3.0f);
					RecreateViews();
				}, []() {
				...
				System_LaunchUrl(LaunchUrlType::BROWSER_URL, "market://details?id=org.nesplay_psp.nesplay_pspgold");
#else
