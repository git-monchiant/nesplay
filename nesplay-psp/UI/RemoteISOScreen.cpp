...
static const char * const REPORT_HOSTNAME = "report.nesplay_psp.org";
...
bool RemoteISOConnectScreen::FindServer(std::string &resultHost, int &resultPort) {
	...
	http.SetUserAgent(StringFromFormat("nesplay-psp/%s", NESPLAY_PSP_GIT_VERSION));
...
static bool LoadGameList(const Path &url, std::vector<Path> &games) {
	...
	browser.SetUserAgent(StringFromFormat("nesplay-psp/%s", NESPLAY_PSP_GIT_VERSION));
...
void RemoteISOScreen::CreateSettingsTab(UI::ViewGroup *remoteisoSettings) {
	...
	remoteisoSettings->Add(new CheckBox(&g_Config.bRemoteShareOnStartup, ri->T("Share on NESPLAY_PSP startup")));
