...
#if NESPLAY_PSP_PLATFORM(IOS) || NESPLAY_PSP_PLATFORM(MAC)
...
public:
	...
	void GetContentDimensions(const UIContext &dc, float &w, float &h) const override {
		if (gridStyle_) {
			...
			h = 296*g_Config.fGameGridScale;  // 256 image + 40 title area
		} else {
			w = 500;
			h = 50;
		}
...
void GameButton::Draw(UIContext &dc) {
	...
	case IdentifiedFileType::NESPLAY_PSP_GE_DUMP: imageIcon = ImageID("I_DISPLAY"); break;
	...
	case IdentifiedFileType::NESPLAY_PSP_SAVESTATE:
	...
	// Reserve space for title text below the image in grid mode
	float titleAreaHeight = 0;
	if (gridStyle_) {
		titleAreaHeight = 40 * g_Config.fGameGridScale;
		h -= (int)titleAreaHeight;
	}

	if (HasFocus())
		style = down_ ? dc.GetTheme().itemDownStyle : dc.GetTheme().itemFocusedStyle;

	// Steam OS focus effect: bright border around card image area
	if (gridStyle_ && HasFocus()) {
		float borderSize = 3.0f * g_Config.fGameGridScale;
		dc.Draw()->Flush();
		dc.RebindTexture();
		Bounds borderBounds(bounds_.x - borderSize, bounds_.y - borderSize,
			(float)bounds_.w + borderSize * 2, (float)h + borderSize * 2);
		dc.FillRect(Drawable(0xFFFFFFFF), borderBounds);
		dc.Draw()->Flush();
	}

	// Draw card background (image area only in grid mode)
	{
		Drawable bg = style.background;
		dc.Draw()->Flush();
		dc.RebindTexture();
		if (gridStyle_) {
			Bounds cardBounds(bounds_.x, bounds_.y, (float)bounds_.w, (float)h);
			dc.FillRect(bg, cardBounds);
		} else {
	...
	if (texture) {
		...
		// Aspect-fit: center image within card without stretching
		float scaleW = (float)w / tw;
		float scaleH = (float)h / th;
	...
	// Draw game title below image only for focused card (Steam Deck style)
	if (gridStyle_ && titleAreaHeight > 0 && HasFocus()) {
		const std::string currentTitle = ginfo->GetTitle();
		if (!currentTitle.empty()) {
			// Clean title: strip region/format tags like (USA), (Europe), (PSP), (PSN), etc.
			std::string clean = ReplaceAll(currentTitle, "\n", " ");
			// Remove parenthesized tags
			std::string result;
			int depth = 0;
			for (char c : clean) {
				if (c == '(') depth++;
			...
			title_ = result.empty() ? clean : result;
		}
		if (!title_.empty()) {
			dc.Draw()->Flush();
			dc.RebindTexture();
			float titleY = bounds_.y + bounds_.h - titleAreaHeight;
			float fontScale = 0.45f * g_Config.fGameGridScale;
			dc.SetFontScale(fontScale, fontScale);
			// Show full title left-aligned below focused card
			dc.DrawText(title_, bounds_.x + 4.0f, titleY + titleAreaHeight / 2,
				style.fgColor, ALIGN_LEFT | ALIGN_VCENTER);
			dc.SetFontScale(1.0f, 1.0f);
			dc.Draw()->Flush();
		}
	}
...
GameBrowser::GameBrowser(int token, const Path &path, BrowseFlags browseFlags, bool portrait, bool *gridStyle, ScreenManager *screenManager, std::string_view lastText, std::string_view lastLink, UI::LayoutParams *layoutParams)
	...
	using namespace UI;
	path_.SetUserAgent(StringFromFormat("nesplay-psp/%s", NESPLAY_PSP_GIT_VERSION));
...
#if NESPLAY_PSP_PLATFORM(ANDROID) || NESPLAY_PSP_PLATFORM(SWITCH) || defined(USING_WIN_UI) || NESPLAY_PSP_PLATFORM(UWP) || NESPLAY_PSP_PLATFORM(IOS)
...
bool GameBrowser::DisplayTopBar() {
	return false;  // Kiosk mode: no navigation bar
}
...
void GameBrowser::FocusFirstGame() {
	if (!gameList_) return;
	for (int i = 0; i < gameList_->GetNumSubviews(); i++) {
		UI::View *v = gameList_->GetViewByIndex(i);
		if (v->CanBeFocused()) {
...
void GameBrowser::Refresh() {
	...
	if (DisplayTopBar()) {
...
#if NESPLAY_PSP_PLATFORM(IOS_APP_STORE)
...
#elif NESPLAY_PSP_PLATFORM(IOS) || NESPLAY_PSP_PLATFORM(MAC)
		...
		if (*gridStyle_) {
			gameList_ = new UI::GridLayoutList(UI::GridLayoutSettings(154*g_Config.fGameGridScale, 306*g_Config.fGameGridScale), new LinearLayoutParams(FILL_PARENT, WRAP_CONTENT, Margins(10, 0, 0, 0)));
		} else {
	...
	if (HasSpecialFiles(filenames)) {
		...
		std::set<std::string> addedGameNames;  // Deduplicate by filename
		...
		for (size_t i = 0; i < fileInfo.size(); i++) {
			...
			if (!isGame && !isSaveData) {
				// Scan one level deeper for game files (e.g. roms/<folder>/<game>.iso)
				if (fileInfo[i].isDirectory) {
					std::vector<File::FileInfo> subFiles;
					File::GetFilesInDir(fileInfo[i].fullName, &subFiles, "iso:cso:chd:pbp:");
					for (auto &sf : subFiles) {
						if (!sf.isDirectory) {
							std::string baseName = sf.name;
							if (addedGameNames.find(baseName) == addedGameNames.end()) {
								addedGameNames.insert(baseName);
								gameButtons.push_back(new GameButton(sf.fullName, *gridStyle_, new UI::LinearLayoutParams(*gridStyle_ == true ? UI::WRAP_CONTENT : UI::FILL_PARENT, UI::WRAP_CONTENT)));
							}
						}
					}
				}
				...
				std::string baseName = fileInfo[i].name;
				if (addedGameNames.find(baseName) == addedGameNames.end()) {
					addedGameNames.insert(baseName);
...
constexpr std::string_view getGamesUri = "https://www.nesplay_psp.org/getgames_ios";
constexpr std::string_view getHomebrewUri = "https://www.nesplay_psp.org/gethomebrew_ios";
#else
constexpr std::string_view getGamesUri = "https://www.nesplay_psp.org/getgames";
constexpr std::string_view getHomebrewUri = "https://www.nesplay_psp.org/gethomebrew";
#endif
constexpr std::string_view remoteGamesUri = "https://www.nesplay_psp.org/docs/reference/disc-streaming";
...
public:
	...
	void Draw(UIContext &dc) override {
		...
		if (logoImg && bounds_.w >= iconImg->w + logoImg->w + 36) {
			dc.Draw()->DrawImage(ImageID("I_LOGO"), bounds_.x + iconImg->w + 8, bounds_.y + 4, 1.0f);
		}

		dc.Flush();

		const FontStyle *style = GetTextStyle(dc, TextSize::Tiny);
		dc.SetFontStyle(*style);
		dc.DrawText("1.0.0",
			bounds_.x + iconImg->w + 8,
			bounds_.y + (logoImg ? logoImg->h + 8 : iconImg->h - 12),
	...
	bool Touch(const TouchInput &touch) override {
		return UI::AnchorLayout::Touch(touch);
	}
...
void MainScreen::CreateMainButtons(UI::ViewGroup *parent, bool portrait) {
	...
	parent->Add(portrait ? new Choice(ImageID("I_INFO"), portrait ? new LinearLayoutParams() : nullptr) : new Choice(mm->T("About NESPLAY_PSP")))->OnClick.Handle(this, &MainScreen::OnCredits);

	if (!portrait) {
		parent->Add(new Choice(mm->T("www.nesplay_psp.org")))->OnClick.Handle(this, &MainScreen::OnNESPLAY_PSPOrg);
	}

	if (!System_GetPropertyBool(SYSPROP_APP_GOLD) && (System_GetPropertyInt(SYSPROP_DEVICE_TYPE) != DEVICE_TYPE_VR)) {
		Choice *gold = parent->Add(portrait ? new Choice(ImageID("I_ICON_GOLD"), portrait ? new LinearLayoutParams() : nullptr) : new Choice(mm->T("Buy NESPLAY_PSP Gold")));
...
#elif NESPLAY_PSP_PLATFORM(ANDROID)
...
void MainScreen::CreateViews() {
	...
	const bool vertical = false;  // Always use kiosk layout (no header)
	...
	if (hasStorageAccess) {
		CreateBrowserTab(Path(g_Config.currentDirectory), "Games", "", "", BrowseFlags::NONE, &g_Config.bGridView2, &g_Config.fGameListScrollPosition);
		...
		if (storageIsTemporary) {
			...
			leftColumn->Add(new TextView(mm->T("SavesAreTemporary", "NESPLAY_PSP saving in temporary storage"), ALIGN_HCENTER, false));
			leftColumn->Add(new TextView(mm->T("SavesAreTemporaryGuidance", "Extract NESPLAY_PSP somewhere to save permanently"), ALIGN_HCENTER, false));
		...
		LinearLayout *buttonHolder = new LinearLayout(ORIENT_HORIZONTAL, new LinearLayoutParams(WRAP_CONTENT, WRAP_CONTENT));
		buttonHolder->Add(new Spacer(new LinearLayoutParams(1.0f)));
		focusButton = new Button(mm->T("Give NESPLAY_PSP permission to access storage"), new LinearLayoutParams(WRAP_CONTENT, WRAP_CONTENT));
		focusButton->SetPadding(32, 16);
		...
		leftColumn->Add(buttonHolder);
		leftColumn->Add(new Spacer(10.0f));
		leftColumn->Add(new TextView(mm->T("NESPLAY_PSP can't load games or save right now"), ALIGN_HCENTER, false));
		leftColumn->Add(new Spacer(new LinearLayoutParams(0.1f)));
	}
	...
	if (vertical) {
		...
		// Steam OS kiosk style layout with overlay sidebar
		AnchorLayout *rootAnchor = new AnchorLayout(new LayoutParams(FILL_PARENT, FILL_PARENT));

		// Main content: game grid + bottom guide bar
		LinearLayout *contentLayout = new LinearLayout(ORIENT_VERTICAL, new AnchorLayoutParams(FILL_PARENT, FILL_PARENT));
		rootAnchor->Add(contentLayout);

		// Game grid fills most of screen
		leftColumn->ReplaceLayoutParams(new LinearLayoutParams(1.0f));
		contentLayout->Add(leftColumn);

		// Bottom guide bar
		LinearLayout *guideBar = new LinearLayout(ORIENT_HORIZONTAL, new LinearLayoutParams(FILL_PARENT, 44.0f, Margins(0, 0, 0, 0)));
		guideBar->SetBG(Drawable(0xFF101018));
		guideBar->SetSpacing(0.0f);
		contentLayout->Add(guideBar);

		guideBar->Add(new Spacer(20.0f));
		TextView *menuHint = guideBar->Add(new TextView("L2  MENU", ALIGN_VCENTER, false, new LinearLayoutParams(WRAP_CONTENT, FILL_PARENT)));
		menuHint->SetSmall(true);
		guideBar->Add(new Spacer(1.0f, new LinearLayoutParams(1.0f)));
		TextView *selectHint = guideBar->Add(new TextView("A  SELECT", ALIGN_VCENTER, false, new LinearLayoutParams(WRAP_CONTENT, FILL_PARENT)));
		selectHint->SetSmall(true);
		guideBar->Add(new Spacer(20.0f));
		TextView *backHint = guideBar->Add(new TextView("B  BACK", ALIGN_VCENTER, false, new LinearLayoutParams(WRAP_CONTENT, FILL_PARENT)));
		backHint->SetSmall(true);
		guideBar->Add(new Spacer(20.0f));

		// Kiosk: hide tab strip, show only game grid
		tabHolder_->SetCurrentTab(showRecent ? 1 : 0, true);
		// Hide first 2 subviews (tab scroll + separator)
		for (int i = 0; i < std::min(tabHolder_->GetNumSubviews(), 2); i++) {
			tabHolder_->GetViewByIndex(i)->SetVisibility(V_GONE);
			tabHolder_->GetViewByIndex(i)->SetEnabled(false);
		}
	...
	} else if (!gameBrowsers_.empty()) {
		// Focus the first game in the active browser
		int idx = (showRecent && gameBrowsers_.size() > 1) ? 1 : 0;
		root_->SetDefaultFocusView(gameBrowsers_[idx]);
	} else if (tabHolder_->GetVisibility() != V_GONE) {
		root_->SetDefaultFocusView(tabHolder_);
	}

	// Enable focus movement for controller/kiosk navigation.
	// Auto-focus is handled in update() after browser finishes loading.
	UI::EnableFocusMovement(true);
	needsAutoFocus_ = true;

	root_->SetTag("mainroot");
}


void MainScreen::OpenSettings() {
	screenManager()->push(new GameSettingsScreen(Path()));
}

void MainScreen::HandleL2(bool pressed) {
	if (pressed && !l2WasDown_) {
		l2WasDown_ = true;
		OpenSettings();
	} else if (!pressed) {
		l2WasDown_ = false;
...
bool MainScreen::key(const KeyInput &touch) {
	if (touch.keyCode == NKCODE_ESCAPE) {
		HandleL2(!!(touch.flags & KeyInputFlags::DOWN));
		return true;
	}
	if (touch.keyCode == NKCODE_BUTTON_L2) {
		// Consume but don't process. L2 toggle state is managed by axis handler only,
		// because KEY UP events are broadcast to all screen layers and create stale state.
		return true;
	}

	// Search disabled for kiosk mode

	return UIBaseScreen::key(touch);
}

void MainScreen::axis(const AxisInput &axis) {
	if (axis.axisId == JOYSTICK_AXIS_LTRIGGER) {
		HandleL2(axis.value > 0.5f);
	}
	UIBaseScreen::axis(axis);
}
...
void MainScreen::update() {
	...
	// Auto-focus first game once the active browser has loaded.
	// Done here (not in GameBrowser) so icons have time to load before focus highlight appears.
	if (needsAutoFocus_ && UI::IsFocusMovementEnabled() && !gameBrowsers_.empty()) {
		int tab = tabHolder_->GetCurrentTab();
		if (tab >= 0 && tab < (int)gameBrowsers_.size() && gameBrowsers_[tab]->IsLoaded()) {
			gameBrowsers_[tab]->FocusFirstGame();
			needsAutoFocus_ = false;
		}
	}
...
void MainScreen::OnGameHighlight(UI::EventParams &e) {
	...
	// Kiosk mode: don't play game audio on highlight
...
#if NESPLAY_PSP_PLATFORM(IOS_APP_STORE)
		System_LaunchUrl(LaunchUrlType::BROWSER_URL, "https://www.nesplay_psp.org/buygold_ios");
#else
		System_LaunchUrl(LaunchUrlType::BROWSER_URL, "https://www.nesplay_psp.org/buygold");
#endif
	}
}

void MainScreen::OnNESPLAY_PSPOrg(UI::EventParams &e) {
	System_LaunchUrl(LaunchUrlType::BROWSER_URL, "https://www.nesplay_psp.org");
}

void MainScreen::OnForums(UI::EventParams &e) {
	System_LaunchUrl(LaunchUrlType::BROWSER_URL, "https://forums.nesplay_psp.org");
}
...
void MainScreen::dialogFinished(const Screen *dialog, DialogResult result) {
	...
	if (tag == "GameSettings") {
		// l2WasDown_ is reset by HandleL2 on actual release, not here.
		// Restore focus to the previously highlighted game.
		if (!highlightedGamePath_.empty()) {
			int tab = tabHolder_->GetCurrentTab();
			if (tab >= 0 && tab < (int)gameBrowsers_.size()) {
				gameBrowsers_[tab]->FocusGame(highlightedGamePath_);
			}
		}
	} else if (tag == "Store") {
		backFromStore_ = true;
...
void UmdReplaceScreen::CreateViews() {
	...
	GameBrowser *tabAllGames = new GameBrowser(GetRequesterToken(), Path(g_Config.currentDirectory), BrowseFlags::STANDARD, portrait, &g_Config.bGridView2, screenManager(),
		mm->T("How to get games"), "https://www.nesplay_psp.org/getgames.html",
		new LinearLayoutParams(FILL_PARENT, FILL_PARENT));
