#!/bin/bash
#ffmpeg for windows arm64
#windows_arm64-build.sh requirements.
# MinGW
#
...
# 3. $ cd /PathTo/nesplay_psp/ffmpeg
# 4. $ windows_arm64-build.sh
#build requements.
# use toolchain=msvc
...
echo "---- rename and copy for nesplay_psp ----"
if (isstaticlib) then
    pushd $PREFIX/lib
    ...
    for fname in *.def; do
        ...
        if (isintobin) then
            ...
            lib \/machine:ARM64 \/def:$fname \/out:$abspath
            popd
        	rm -fv $binpath/$outname.exp
        	pushd $libpath
        else
            lib \/machine:ARM64 \/def:$fname \/out:$outname.lib
            rm -fv $outname.exp
        fi
    done
    popd
fi
echo "---- windows_arm64-build.sh finished ----"
}
build_ffmpeg
