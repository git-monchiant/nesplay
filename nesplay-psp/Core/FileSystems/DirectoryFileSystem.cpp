...
#if !NESPLAY_PSP_PLATFORM(WINDOWS) && !defined(HAVE_LIBRETRO_VFS)
	if (success) {
...
int DirectoryFileSystem::OpenFile(std::string filename, FileAccess access, const char *devicename) {
	...
	if (err != 0) {
		...
		logError = (int)errno;
#endif
		if (!(access & FILEACCESS_NESPLAY_PSP_QUIET)) {
	...
	for (auto &file : files) {
		...
		if (hideISOFiles) {
			if (endsWithNoCase(entry.name, ".cso") || endsWithNoCase(entry.name, ".iso") || endsWithNoCase(entry.name, ".chd")) {  // chd not really necessary, but let's hide them too.
				...
				} else if (file.name == "GAME" || file.name == "TEXTURES" || file.name == "NESPLAY_PSP_STATE" || file.name == "PLUGINS" || file.name == "SYSTEM" || equalsNoCase(file.name, "Cheats")) {
