...
static Path SymbolMapFilename(const Path &currentFilename, const char *ext) {
	...
	if (info.isDirectory) {
		return currentFilename / (std::string(".nesplay_psp-symbols") + ext);
	}
...
static void GetBootError(IdentifiedFileType type, std::string *errorString) {
	...
	case IdentifiedFileType::NESPLAY_PSP_SAVESTATE: *errorString = "This is a saved state, not a game."; break; // Actually, we could make it load it...
	case IdentifiedFileType::PSP_SAVEDATA_DIRECTORY: *errorString = "This is save data, not a game."; break;
	case IdentifiedFileType::PSP_PS1_PBP: *errorString = "PS1 EBOOTs are not supported by NESPLAY_PSP."; break;
	case IdentifiedFileType::PSP_UMD_VIDEO_ISO: *errorString = "UMD Video ISOs are not supported by NESPLAY_PSP."; break;
...
static bool CPU_Init(FileLoader *fileLoader, IdentifiedFileType type, std::string *errorString) {
	...
	case IdentifiedFileType::PSP_DISC_DIRECTORY:
		...
		if (type == IdentifiedFileType::PSP_DISC_DIRECTORY) {
			// Check for existence of nesplay_psp-index.lst - if it exists, the user likely knows what they're doing.
	...
	{
		ERROR_LOG(Log::Loader, "NESPLAY_PSP doesn't support UMD Video.");
		auto er = GetI18NCategory(I18NCat::ERRORS);
		*errorString = er->T("NESPLAY_PSP doesn't support UMD Video.");
		return false;
	}
	default:
	{
	...
	if (type != IdentifiedFileType::NESPLAY_PSP_GE_DUMP) {
		HLEPlugins::Init();
	}
...
bool PSP_InitStart(const CoreParameter &coreParam) {
	...
	g_loadingThread = std::thread([errorString]() {
		...
		NOTICE_LOG(Log::Boot, "NESPLAY_PSP %s", NESPLAY_PSP_GIT_VERSION);
