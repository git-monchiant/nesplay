// Copyright (c) 2020 NESPLAY_PSP Project.
...
#if !NESPLAY_PSP_PLATFORM(WINDOWS) && !NESPLAY_PSP_PLATFORM(ANDROID) && !defined(__LIBRETRO__) && !NESPLAY_PSP_PLATFORM(SWITCH)
#include <unistd.h>
#include <sys/types.h>
#include <sys/mman.h>
#include <sys/stat.h>
...
#define ID_SHM_NAME "/NESPLAY_PSP_ID"
...
static bool UpdateInstanceCounter(void (*callback)(volatile InstanceInfo *)) {
	...
	return result;
#elif NESPLAY_PSP_PLATFORM(ANDROID) || defined(__LIBRETRO__) || NESPLAY_PSP_PLATFORM(SWITCH)
...
// Get current number of instance of NESPLAY_PSP running.
...
void InitInstanceCounter() {
	...
	mapLock = CreateMutex(nullptr, FALSE, L"NESPLAY_PSP_ID_mutex");
