...
int g_screenshotFailures;
	...
	void SaveStart::DoState(PointerWrap &p) {
		...
		if (s >= 2) {
			...
			if (saveStateInitialGitVersion.empty())
				saveStateInitialGitVersion = NESPLAY_PSP_GIT_VERSION;
	...
	bool IsOldVersion() {
		...
		Version gitVer(NESPLAY_PSP_GIT_VERSION);
	...
	static Status TriggerLoadWarnings(std::string &callbackMessage) {
		...
		if (IsOldVersion()) {
			// Save states also preserve bugs from old NESPLAY_PSP versions, so warn.
			callbackMessage = sc->T("Loaded. Save in game, restart, and load for less bugs.");
			return Status::SUCCESS;
		}
	...
	void Process() {
		...
		for (const auto &op : operations) {
			...
			case OperationType::Save:
				...
				result = CChunkFileReader::Save(op.filename, title, NESPLAY_PSP_GIT_VERSION, state);
				if (result == CChunkFileReader::ERROR_NONE) {
