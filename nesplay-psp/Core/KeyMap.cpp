...
void RestoreDefault() {
	...
	// Kiosk: ensure L2/LT triggers Pause on ALL device types and key codes
	// XInput sends JOYSTICK_AXIS_LTRIGGER, DInput sends JOYSTICK_AXIS_Z or button codes
	for (int i = 0; i <= 3; i++) {
		InputDeviceID xinputDev = (InputDeviceID)((int)DEVICE_ID_XINPUT_0 + i);
		g_controllerMap[VIRTKEY_PAUSE].push_back(MultiInputMapping(InputMapping(xinputDev, NKCODE_BUTTON_L2)));
		g_controllerMap[VIRTKEY_PAUSE].push_back(MultiInputMapping(InputMapping(xinputDev, JOYSTICK_AXIS_LTRIGGER, 1)));
	}
	for (int i = 0; i <= 3; i++) {
		InputDeviceID padDev = (InputDeviceID)((int)DEVICE_ID_PAD_0 + i);
		g_controllerMap[VIRTKEY_PAUSE].push_back(MultiInputMapping(InputMapping(padDev, NKCODE_BUTTON_L2)));
		g_controllerMap[VIRTKEY_PAUSE].push_back(MultiInputMapping(InputMapping(padDev, NKCODE_BUTTON_7)));  // DInput L2
		g_controllerMap[VIRTKEY_PAUSE].push_back(MultiInputMapping(InputMapping(padDev, JOYSTICK_AXIS_LTRIGGER, 1)));
		g_controllerMap[VIRTKEY_PAUSE].push_back(MultiInputMapping(InputMapping(padDev, JOYSTICK_AXIS_Z, 1)));  // DInput L2 axis
		g_controllerMap[VIRTKEY_PAUSE].push_back(MultiInputMapping(InputMapping(padDev, JOYSTICK_AXIS_BRAKE, 1)));
	}
