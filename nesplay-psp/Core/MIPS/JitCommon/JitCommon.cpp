...
#if NESPLAY_PSP_ARCH(ARM)
#include "../ARM/ArmJit.h"
#elif NESPLAY_PSP_ARCH(ARM64)
#include "../ARM64/Arm64Jit.h"
#include "../ARM64/Arm64IRJit.h"
#elif NESPLAY_PSP_ARCH(X86) || NESPLAY_PSP_ARCH(AMD64)
#include "../x86/Jit.h"
#include "../x86/X64IRJit.h"
#elif NESPLAY_PSP_ARCH(RISCV64)
#include "../RiscV/RiscVJit.h"
#elif NESPLAY_PSP_ARCH(LOONGARCH64)
...
#if NESPLAY_PSP_PLATFORM(WINDOWS) && !defined(__LIBRETRO__)
#define DISASM_ALL 1
#endif

#if NESPLAY_PSP_ARCH(ARM) || defined(DISASM_ALL)
...
#if NESPLAY_PSP_ARCH(ARM64) || defined(DISASM_ALL)
...
#if NESPLAY_PSP_ARCH(X86) || NESPLAY_PSP_ARCH(AMD64)

const char *nesplay_psp_resolver(struct ud*,
	uint64_t addr,
	int64_t *offset) {
...
std::vector<std::string> DisassembleX86(const u8 *data, int size) {
	...
	ud_set_sym_resolver(&ud_obj, &nesplay_psp_resolver);
...
#if NESPLAY_PSP_ARCH(RISCV64) || defined(DISASM_ALL)
...
#if NESPLAY_PSP_ARCH(LOONGARCH64) || defined(DISASM_ALL)
