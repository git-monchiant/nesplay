// Copyright (c) 2012- NESPLAY_PSP Project.
...
#if NESPLAY_PSP_ARCH(ARM64_NEON) || NESPLAY_PSP_ARCH(SSE2)
...
#if NESPLAY_PSP_ARCH(ARM) || NESPLAY_PSP_ARCH(ARM64) || NESPLAY_PSP_ARCH(X86) || NESPLAY_PSP_ARCH(AMD64) || NESPLAY_PSP_ARCH(RISCV64) || NESPLAY_PSP_ARCH(LOONGARCH64)
...
#if NESPLAY_PSP_PLATFORM(WINDOWS) || NESPLAY_PSP_PLATFORM(LINUX) || NESPLAY_PSP_PLATFORM(MAC)
	return -1;
#else
	return 0;
#endif
...
#if NESPLAY_PSP_PLATFORM(ANDROID) || defined(USING_QT_UI) || NESPLAY_PSP_PLATFORM(UWP) || NESPLAY_PSP_PLATFORM(IOS)
...
#elif NESPLAY_PSP_PLATFORM(WINDOWS)
...
#elif NESPLAY_PSP_PLATFORM(IOS_APP_STORE)
	return (int)GPUBackend::VULKAN;
#endif
...
bool Config::IsBackendEnabled(GPUBackend backend) {
	...
	if (backend != GPUBackend::DIRECT3D11)
		return false;
#elif NESPLAY_PSP_PLATFORM(SWITCH)
...
static std::string DefaultInfrastructureUsername() {
	// If the user has already picked a Nickname that satisfies the rules and is not "NESPLAY_PSP",
	// let's use that.
	// NOTE: This type of dependency means that network settings must be AFTER system settings in sections[].
	if (g_Config.sNickName != "NESPLAY_PSP" &&
...
#if NESPLAY_PSP_PLATFORM(ANDROID) && NESPLAY_PSP_ARCH(ARM64)
...
static const ConfigSetting systemParamSettings[] = {
	...
	ConfigSetting("NickName", SETTING(g_Config, sNickName), "NESPLAY_PSP", CfgFlag::PER_GAME),
...
#if defined(USING_WIN_UI) || defined(USING_QT_UI) || NESPLAY_PSP_PLATFORM(ANDROID) || NESPLAY_PSP_PLATFORM(SWITCH)
...
void Config::UpdateIniLocation(const char *iniFileName, const char *controllerIniFilename) {
	...
	const char *nesplay_pspIniFilename = "nesplay-psp.ini";
	bool exists;
	iniFilename_ = FindConfigFile(searchPath_, useIniFilename ? iniFileName : nesplay_pspIniFilename, &exists);
...
void Config::Load(const char *iniFileName, const char *controllerIniFilename) {
	...
	g_DownloadManager.SetUserAgent(StringFromFormat("NESPLAY_PSP/%s", NESPLAY_PSP_GIT_VERSION));
...
void Config::PostLoadCleanup() {
	// Override nesplay_psp.ini JIT value to prevent crashing
	...
	if (NESPLAY_PSP_ID > 1) {
		NOTICE_LOG(Log::Audio, "Secondary instance %d - silencing audio", (int)NESPLAY_PSP_ID);
...
void Config::PostSaveCleanup() {
	if (jitForcedOff) {
		// Force JIT off again just in case Config::Save() is called without exiting NESPLAY_PSP.
