...
IdentifiedFileType Identify_File(FileLoader *fileLoader, std::string *errorString) {
	...
	if (extension == ".iso" || extension == ".cso" || extension == ".chd") {
		...
		return IdentifiedFileType::NESPLAY_PSP_SAVESTATE;
	} else if (extension == ".ppdmp") {
		char data[8]{};
		fileLoader->ReadAt(0, 8, data);
		if (memcmp(data, "NESPLAY_PSPGE", 8) == 0) {
			return IdentifiedFileType::NESPLAY_PSP_GE_DUMP;
		}
	}
...
const char *IdentifiedFileTypeToString(IdentifiedFileType type) {
	...
	case IdentifiedFileType::NESPLAY_PSP_SAVESTATE: return "NESPLAY_PSP_SAVESTATE";
	case IdentifiedFileType::NESPLAY_PSP_GE_DUMP: return "NESPLAY_PSP_GE_DUMP";
