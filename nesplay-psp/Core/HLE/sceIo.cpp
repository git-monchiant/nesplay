...
public:
	...
	static int GetStaticIDType() { return NESPLAY_PSP_KERNEL_TMID_File; }
	int GetIDType() const override { return NESPLAY_PSP_KERNEL_TMID_File; }
...
static u32 sceIoChstat(const char *filename, u32 iostatptr, u32 changebits) {
	...
	if (changebits & SCE_CST_ATTR) {
		// These are pretty much all of the reported calls: https://report.nesplay_psp.org/logs/kind/1115
...
static FileNode *__IoOpen(int &error, const char *filename, int flags, int mode) {
	...
	if (startsWithNoCase(filename, "tty0:")) {
		...
		h = pspFileSystem.OpenFile(filename, (FileAccess)(access | (int)FileAccess::FILEACCESS_NESPLAY_PSP_QUIET));
		if (h < 0) {
			error = h;
			return nullptr;
		}
...
public:
	...
	static u32 GetMissingErrorCode() { return SCE_KERNEL_ERROR_BADF; }
	static int GetStaticIDType() { return NESPLAY_PSP_KERNEL_TMID_DirList; }
	int GetIDType() const override { return NESPLAY_PSP_KERNEL_TMID_DirList; }
