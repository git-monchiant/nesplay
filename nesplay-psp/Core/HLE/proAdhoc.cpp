...
#if NESPLAY_PSP_PLATFORM(SWITCH) && !defined(INADDR_NONE)
...
void getLocalMac(SceNetEtherAddr * addr){
	...
	if (NESPLAY_PSP_ID > 1) {
		memset(&mac, NESPLAY_PSP_ID, sizeof(mac));
...
#if !defined(TCP_KEEPIDLE) && !NESPLAY_PSP_PLATFORM(SWITCH)
...
#if !NESPLAY_PSP_PLATFORM(SWITCH) && !NESPLAY_PSP_PLATFORM(OPENBSD)
...
#endif // !NESPLAY_PSP_PLATFORM(SWITCH) && !NESPLAY_PSP_PLATFORM(OPENBSD)
	return result;
}
