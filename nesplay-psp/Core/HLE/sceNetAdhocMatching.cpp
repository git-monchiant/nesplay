// Copyright (c) 2025- NESPLAY_PSP Project.
...
void sendDeathPacket(SceNetAdhocMatchingContext * context, SceNetEtherAddr * mac) {
	...
	SceNetAdhocMatchingMemberInternal * peer = context->peerlist;
	for (; peer != NULL; peer = peer->next) {
		// Skip dead Child? Or May be we should also tells the disconnected Child, that they have been disconnected from the Host (in the case they were disconnected because they went to NESPLAY_PSP settings for too long)
		if (peer == deadkid) {
			// Set Opcode
