...
int sceNetInetSelect(int nfds, u32 readfdsPtr, u32 writefdsPtr, u32 exceptfdsPtr, u32 timeoutPtr) {
	...
	// TODO: Simulate blocking behaviour when timeout = NULL to prevent NESPLAY_PSP from freezing
...
int sceNetInetPoll(u32 fdsPtr, u32 nfds, int timeout) { // timeout in miliseconds just like posix poll? or in microseconds as other PSP timeout?
	...
	// TODO: Simulate blocking behaviour when timeout is non-zero to prevent NESPLAY_PSP from freezing
...
static int sceNetInetSetsockopt(int socket, int level, int optname, u32 optvalPtr, int optlen) {
	...
	if (level == PSP_NET_INET_SOL_SOCKET) {
		...
		// TODO: Ignoring SO_NOSIGPIPE flag to prevent crashing NESPLAY_PSP (not sure if PSP has NOSIGPIPE or not tho, defined as 0x1022 on Darwin)
...
static int sceNetInetBind(int socket, u32 namePtr, int namelen) {
	...
	// TODO: Make use Port Offset only for NESPLAY_PSP to NESPLAY_PSP communications (ie. IP addresses available in the group/friendlist), otherwise should be considered as Online Service thus should use the port as is.
