...
#if defined(_WIN32) && !NESPLAY_PSP_PLATFORM(UWP) && !defined(__LIBRETRO__)
...
std::vector<std::string> Camera::getDeviceList() {
...
#elif NESPLAY_PSP_PLATFORM(ANDROID) || NESPLAY_PSP_PLATFORM(IOS)
	...
	#elif NESPLAY_PSP_PLATFORM(LINUX) // SDL:Linux
...
int Camera::startCapture() {
	...
	#elif NESPLAY_PSP_PLATFORM(ANDROID) || NESPLAY_PSP_PLATFORM(IOS) || defined(USING_QT_UI)
		char command[40] = {0};
