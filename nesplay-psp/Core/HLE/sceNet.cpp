...
static const std::string_view jsonUrl = "http://metadata.nesplay_psp.org/infra-dns.json";
...
void InitLocalhostIP() {
	...
	uint32_t localIP = 0x7F000001 + NESPLAY_PSP_ID - 1;
...
static u32 sceWlanGetEtherAddr(u32 addrAddr) {
	...
	if (NESPLAY_PSP_ID > 1) {
		Memory::Memset(addrAddr, NESPLAY_PSP_ID, 6);
