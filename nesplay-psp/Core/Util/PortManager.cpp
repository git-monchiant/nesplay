...
PortManager::~PortManager() {
	// FIXME: On Windows it seems using any UPnP functions in this destructor that gets triggered when exiting NESPLAY_PSP will resulting to UPNPCOMMAND_HTTP_ERROR due to early WSACleanup (miniupnpc was getting WSANOTINITIALISED internally)
}
...
bool PortManager::Initialize(const unsigned int timeout) {
	...
	if (devlist)
	{
		...
		m_defaultDesc = "NESPLAY_PSP:" + gameID + ":" + g_Config.sNickName; // Some routers may automatically prefixed it with "UPnP:"
...
bool PortManager::Add(const char* protocol, unsigned short port, unsigned short intport) {
	...
	// Only add new port map if it's not previously created by NESPLAY_PSP for current IP
...
bool PortManager::Restore() {
	...
	for (auto it = m_otherPortList.begin(); it != m_otherPortList.end(); ++it) {
		if (it->taken) {
			...
			// Remove it first if it's still being taken by NESPLAY_PSP
			auto el_it = std::find_if(m_portList.begin(), m_portList.end(),
				[port_str, protocol](const std::pair<std::string, std::string>& el) { return el.first == port_str && el.second == protocol; });
			if (el_it != m_portList.end()) {
...
bool PortManager::Clear() {
	...
	do {
		...
		// May gets UPNPCOMMAND_HTTP_ERROR when called while exiting NESPLAY_PSP (ie. used in destructor)
		r = UPNP_GetGenericPortMappingEntry(urls->controlURL,
			...
			rHost, duration);
		// Only removes port mappings created by NESPLAY_PSP for current LAN IP
		if (r == 0 && intAddr == m_lanip && std::string(desc).find("NESPLAY_PSP:") != std::string::npos) {
...
bool PortManager::RefreshPortList() {
	...
	do {
		...
		if (r == 0) {
			...
			// Only include port mappings created by NESPLAY_PSP for current LAN IP
			if (intAddr == m_lanip && desc_str.find("NESPLAY_PSP:") != std::string::npos) {
				m_portList.push_back({ extPort, protocol });
			}
			// Port mappings belong to others that might be taken by NESPLAY_PSP later
			else {
