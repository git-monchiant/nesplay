...
Path GetSysDirectory(PSPDirectories directoryType) {
	...
	case DIRECTORY_SAVESTATE:
		return pspDirectory / "NESPLAY_PSP_STATE";
	...
	case DIRECTORY_COVERS:
		return pspDirectory / "COVERS";
...
Path GetGameConfigFilePath(const Path &searchPath, std::string_view gameId, bool *exists) {
	std::string_view nesplay_pspIniFilename = "_nesplay-psp.ini";
	std::string iniFileName = join(gameId, nesplay_pspIniFilename);
...
#if !NESPLAY_PSP_PLATFORM(ANDROID) && (NESPLAY_PSP_PLATFORM(LINUX) || NESPLAY_PSP_PLATFORM(MAC))
	std::string str = path.ToString();
