...
private:
	// Retrieves port lists mapped by NESPLAY_PSP for current LAN IP & other's applications
	bool RefreshPortList();

	// Removes any lingering mapped ports created by NESPLAY_PSP (including from previous crashes)
	bool Clear();

	// Restore ports mapped by others that were taken by NESPLAY_PSP, better used after Clear()
	bool Restore();
