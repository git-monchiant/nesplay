// Copyright (c) 2017- NESPLAY_PSP Project.
...
// This WebSocket (connected through the same port as disc sharing) allows API/debugger access to NESPLAY_PSP.
// Currently, the only subprotocol "debugger.nesplay_psp.org" uses a simple JSON based interface.
//
// Messages to and from NESPLAY_PSP follow the same basic format:
//    { "event": "NAME", ... }
//
// And are primarily of these types:
//  * Events from the debugger/client (you) to NESPLAY_PSP
//    If there's a response, it will generally use the same name.  It may not be immedate - it's an event.
//  * Spontaneous events from NESPLAY_PSP
//    Things like logs, breakpoint hits, etc. not directly requested.
//
// Otherwise you may see error events which indicate NESPLAY_PSP couldn't understand or failed internally:
//  - "event": "error"
...
void HandleDebuggerRequest(const http::ServerRequest &request) {
	net::WebSocketServer *ws = net::WebSocketServer::CreateAsUpgrade(request, "debugger.nesplay_psp.org");
	if (!ws)
		return;
