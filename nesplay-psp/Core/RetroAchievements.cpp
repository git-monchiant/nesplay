...
static bool HashISOFile(ISOFileSystem *fs, const std::string filename, md5_context *md5) {
	...
	nesplay_psp_md5_update(md5, buffer.get(), sz);
	return true;
}
...
static std::string ComputePSPISOHash(BlockDevice *blockDevice) {
	...
	uint8_t digest[16];
	nesplay_psp_md5_finish(&md5, digest);
...
static std::string ComputePSPHomebrewHash(FileLoader *fileLoader) {
	...
	nesplay_psp_md5_update(&md5, buffer.data(), (int)buffer.size());
...
void Initialize() {
	...
	if (g_Config.bAchievementsEnableRAIntegration) {
		...
		rc_client_begin_load_raintegration(g_rcClient, szFilePath, hWnd, "NESPLAY_PSP", NESPLAY_PSP_GIT_VERSION, &load_integration_callback, hWnd);
		return;
	}
#endif
	TryLoginByToken(true);
