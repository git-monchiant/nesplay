...
std::string System_GetProperty(SystemProperty prop) {
	...
	case SYSPROP_BUILD_VERSION:
		return NESPLAY_PSP_GIT_VERSION;
...
void System_Notify(SystemNotification notification) {
	...
	case SystemNotification::UI:
	{
		...
		if (NESPLAY_PSP_ID >= 1 && peers != g_lastNumInstances) {
...
bool System_MakeRequest(SystemRequestType type, int requestId, const std::string &param1, const std::string &param2, int64_t param3, int64_t param4) {
	...
	{
		const char *name = "nesplay-psp ";
		std::wstring winTitle = ConvertUTF8ToWString(std::string(name) + NESPLAY_PSP_GIT_VERSION);
		if (!param1.empty()) {
	...
	case SystemRequestType::GRAPHICS_BACKEND_FAILED_ALERT:
	{
		...
		std::string_view backendSwitchError = err->T("GenericBackendSwitchCrash", "nesplay-psp crashed while starting. This usually means a graphics driver problem. Try upgrading your graphics drivers.\n\nGraphics backend has been switched:");
...
std::vector<std::wstring> GetWideCmdLine() {
	...
	if (!restartArgs.empty()) {
		std::wstring wargs = ConvertUTF8ToWString("nesplay-psp " + restartArgs);
...
static void InitMemstickDirectory() {
	...
	const Path myDocsPath = rootMyDocsPath / "nesplay-psp";
...
int WINAPI WinMain(HINSTANCE _hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow) {
	...
	// Kiosk mode: force settings (overrides config file)
	g_Config.bFullScreen = true;
	g_Config.bShowMenuBar = false;
	g_Config.iDefaultTab = 1;  // Games tab (skip Recent)
