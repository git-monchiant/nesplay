...
static XInputGetState_t NESPLAY_PSP_XInputGetState = nullptr;
static XInputSetState_t NESPLAY_PSP_XInputSetState = nullptr;
static XInputGetCapabilitiesEx_t NESPLAY_PSP_XInputGetCapabilitiesEx = nullptr;
static DWORD NESPLAY_PSP_XInputVersion = 0;
...
static int LoadXInputDLL() {
	...
	NESPLAY_PSP_XInputVersion = version;
	...
	NESPLAY_PSP_XInputGetState = (XInputGetState_t)GetProcAddress( (HMODULE)s_pXInputDLL, "XInputGetStateEx" );
	if (!NESPLAY_PSP_XInputGetState) {
		NESPLAY_PSP_XInputGetState = (XInputGetState_t)GetProcAddress( (HMODULE)s_pXInputDLL, (LPCSTR)100 );
		if (!NESPLAY_PSP_XInputGetState) {
			NESPLAY_PSP_XInputGetState = (XInputGetState_t)GetProcAddress( (HMODULE)s_pXInputDLL, "XInputGetState" );
		}
	}
	...
	NESPLAY_PSP_XInputSetState = (XInputSetState_t)GetProcAddress((HMODULE)s_pXInputDLL, "XInputSetStateEx");
	if (!NESPLAY_PSP_XInputSetState) {
		NESPLAY_PSP_XInputSetState = (XInputSetState_t)GetProcAddress((HMODULE)s_pXInputDLL, "XInputSetState");
	}
	...
	if (NESPLAY_PSP_XInputVersion >= ((1 << 16) | 4)) {
		NESPLAY_PSP_XInputGetCapabilitiesEx = (XInputGetCapabilitiesEx_t)GetProcAddress((HMODULE)s_pXInputDLL, (LPCSTR)108);
	}
...
static void UnloadXInputDLL() {}
#define NESPLAY_PSP_XInputGetState XInputGetState
#define NESPLAY_PSP_XInputSetState XInputSetState
#endif
...
int XinputDevice::UpdateState() {
	...
	for (int i = 0; i < XUSER_MAX_COUNT; i++) {
		...
		if (padData_[i].checkDelayUpdates-- > 0)
			continue;
		DWORD dwResult = NESPLAY_PSP_XInputGetState(i, &state);
		if (dwResult == ERROR_SUCCESS) {
			if (!padData_[i].connected) {
				...
				if (NESPLAY_PSP_XInputGetCapabilitiesEx != nullptr && NESPLAY_PSP_XInputGetCapabilitiesEx(1, i, 0, &caps) == ERROR_SUCCESS) {
...
void XinputDevice::ApplyVibration(int pad, XINPUT_VIBRATION &vibration) {
	if (PSP_IsInited()) {
		...
		// We have to run NESPLAY_PSP_XInputSetState at time intervals
		...
		if (newVibrationTime_ - prevVibrationTime_ >= 1.0 / 64.0) {
			...
			if (padData_[pad].prevVibration.wLeftMotorSpeed != vibration.wLeftMotorSpeed || padData_[pad].prevVibration.wRightMotorSpeed != vibration.wRightMotorSpeed) {
				NESPLAY_PSP_XInputSetState(pad, &vibration);
				padData_[pad].prevVibration = vibration;
			}
			prevVibrationTime_ = newVibrationTime_;
		}
	} else {
		DWORD dwResult = NESPLAY_PSP_XInputSetState(pad, &vibration);
		if (dwResult != ERROR_SUCCESS) {
