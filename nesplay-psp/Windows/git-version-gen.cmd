...
rem // Always clean up temp file
del "%GIT_VERSION_TEMP%" > NUL 2> NUL

if exist "%GIT_VERSION_FILE%" (
	rem // Skip updating the file if NESPLAY_PSP_GIT_VERSION_NO_UPDATE is 1.
	findstr /B /C:"#define NESPLAY_PSP_GIT_VERSION_NO_UPDATE 1" "%GIT_VERSION_FILE%" > NUL
	if not errorlevel 1 (
		goto gitdone
	)
)
...
if "%GIT_MISSING%" == "1" (
	...
	echo const char *NESPLAY_PSP_GIT_VERSION = "unknown"; >> "%GIT_VERSION_TEMP%"
...
echo // This is a generated file, by git-version-gen.cmd. > "%GIT_VERSION_TEMP%"
echo. >> "%GIT_VERSION_TEMP%"
echo const char *NESPLAY_PSP_GIT_VERSION = "%GIT_VERSION%"; >> "%GIT_VERSION_TEMP%"
echo. >> "%GIT_VERSION_TEMP%"
echo // If you don't want this file to update/recompile, change to 1. >> "%GIT_VERSION_TEMP%"
echo #define NESPLAY_PSP_GIT_VERSION_NO_UPDATE 0 >> "%GIT_VERSION_TEMP%"
...
if exist "%WIN_VERSION_FILE%" (
	rem // Skip updating the file if NESPLAY_PSP_WIN_VERSION_NO_UPDATE is 1.
	findstr /B /C:"#define NESPLAY_PSP_WIN_VERSION_NO_UPDATE 1" "%WIN_VERSION_FILE%" > NUL
	if not errorlevel 1 (
		goto done
	)
)
...
if "%GIT_MISSING%" == "1" (
	...
	echo #define NESPLAY_PSP_WIN_VERSION_STRING "unknown" >> "%WIN_VERSION_TEMP%"
	echo #define NESPLAY_PSP_WIN_VERSION_COMMA 0,0,0,0 >> "%WIN_VERSION_TEMP%"
...
echo. >> "%WIN_VERSION_TEMP%"
echo #define NESPLAY_PSP_WIN_VERSION_STRING "%GIT_VERSION%" >> "%WIN_VERSION_TEMP%"
echo #define NESPLAY_PSP_WIN_VERSION_COMMA %WIN_VERSION_COMMA% >> "%WIN_VERSION_TEMP%"
echo. >> "%WIN_VERSION_TEMP%"
echo // If you don't want this file to update/recompile, change to 1. >> "%WIN_VERSION_TEMP%"
echo #define NESPLAY_PSP_WIN_VERSION_NO_UPDATE 0 >> "%WIN_VERSION_TEMP%"
