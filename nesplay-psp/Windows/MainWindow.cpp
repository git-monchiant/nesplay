...
namespace MainWindow {
	...
	static constexpr const wchar_t *szWindowClass = L"NesplayPSPWnd";
	...
	void Init(HINSTANCE hInstance) {
		...
		wcex.hIcon = LoadIcon(hInstance, (LPCTSTR)IDI_NESPLAY_PSP);
		wcex.hIconSm = (HICON)LoadImage(hInstance, (LPCTSTR)IDI_NESPLAY_PSP, IMAGE_ICON, 16, 16, LR_SHARED);
	...
	void UpdateWindowTitle() {
		...
		if (NESPLAY_PSP_ID >= 1 && GetInstancePeerCount() > 1) {
			title.append(ConvertUTF8ToWString(StringFromFormat(" (instance: %d)", (int)NESPLAY_PSP_ID)));
		}
	...
	bool ConfirmAction(HWND hWnd, bool actionIsReset) {
		// Kiosk mode: always allow exit without confirmation
		return true;
	}
